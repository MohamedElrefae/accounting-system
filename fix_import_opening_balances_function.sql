-- Fix for import_opening_balances RPC function workflow_id constraint error
-- This script finds and fixes the RPC function to use proper approval request creation

-- First, let's find the existing function definition
SELECT 
    routine_name,
    routine_definition
...
-- If the function exists and uses direct INSERT into approval_requests, we need to fix it
-- The fix should replace any direct INSERT with calls to fn_create_approval_request

-- Example of what the fixed function should look like:
...
BEGIN
    ...
    IF v_success_rows > 0 THEN
        v_approval_request_id := public.fn_create_approval_request(
            p_org_id => p_org_id,
            p_target_table => 'opening_balances',
            p_target_id => v_import_id,
            p_requested_by => p_user_id,
