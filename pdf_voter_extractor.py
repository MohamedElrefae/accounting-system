...
Extracts structured voter data from Egyptian election PDFs into relational tables
"""
...
class VoterPDFExtractor:
    ...
    def extract_location_info(self, page_text: str, page_num: int) -> Optional[Dict]:
        ...
        # Common Arabic patterns for location extraction
        patterns = {
    ...
    def extract_voters_from_page(self, page_text: str, location_id: int, page_num: int) -> List[Dict]:
        ...
        # Find voter table start (usually after header info)
        table_start_idx = 0
        for i, line in enumerate(lines):
            # Look for table headers or first voter entry
        ...
        for line in lines[table_start_idx:]:
            ...
            if voter_match:
                ...
                # Handle multi-column layout or wrapped names
        ...
        try:
            with open(pdf_path, 'rb') as file:
                ...
                for page_num in range(total_pages):
                    ...
                    # Extract location info for this page
                    location_info = self.extract_location_info(page_text, page_num + 1)
                    
                    if location_info:
                        # Extract voters for this location
                        ...
                        logger.info(f"Page {page_num + 1}: Found {len(page_voters)} voters for location {location_info['location_number']}")
                    else:
...
""")
            ...
            for location in self.locations:
                values = [
                    ...
                    f"'{location['location_name'].replace(\"'\", \"''\")}'",
                    f"'{location['location_address'].replace(\"'\", \"''\")}'",
                    f"'{location['governorate'].replace(\"'\", \"''\")}'",
                    f"'{location['district'].replace(\"'\", \"''\")}'",
                    f"'{location['main_committee_id']}'",
                    f"'{location['police_department'].replace(\"'\", \"''\")}'",
...
def main():
    parser = argparse.ArgumentParser(description='Extract voter data from Egyptian election PDFs')
    parser.add_argument('pdf_path', help='Path to the PDF file')
    parser.add_argument('--output', '-o', default='output', help='Output directory (default: output)')
    parser.add_argument('--format', '-f', choices=['csv', 'json', 'sql', 'all'], default='all',
    ...
    try:
        ...
        locations, voters = extractor.process_pdf(args.pdf_path)
        ...
        extractor.generate_summary_report(args.output)
