-- Fix for cost analysis using correct column names
-- Need to identify the correct amount column in transactions table
...
-- Assuming transactions table has debit_amount and credit_amount columns
CREATE OR REPLACE VIEW v_cost_analysis_summary AS
SELECT 
    t.id AS transaction_id,
    t.entry_number,
    t.description,
    COALESCE(t.debit_amount, t.credit_amount, 0) AS transaction_amount,
    COALESCE(SUM(tli.total_amount), 0) AS calculated_line_items_total,
    COUNT(tli.id)::INTEGER AS calculated_line_items_count,
    (COALESCE(t.debit_amount, t.credit_amount, 0) - COALESCE(SUM(tli.total_amount), 0)) AS calculated_variance_amount,
    CASE 
        WHEN COALESCE(t.debit_amount, t.credit_amount, 0) = 0 THEN 0::NUMERIC
        ELSE ((COALESCE(t.debit_amount, t.credit_amount, 0) - COALESCE(SUM(tli.total_amount), 0)) / COALESCE(t.debit_amount, t.credit_amount, 0)) * 100
    END AS calculated_variance_pct,
    ABS(COALESCE(t.debit_amount, t.credit_amount, 0) - COALESCE(SUM(tli.total_amount), 0)) < 1.0 AS is_matched,
    ABS(COALESCE(t.debit_amount, t.credit_amount, 0) - COALESCE(SUM(tli.total_amount), 0)) >= 1.0 AS needs_attention,
...
GROUP BY t.id, t.entry_number, t.description, t.debit_amount, t.credit_amount;
...
-- SELECT * FROM v_cost_analysis_summary LIMIT 5;
