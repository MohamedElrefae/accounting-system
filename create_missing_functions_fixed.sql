-- Fixed version for Enterprise Running Balance page
-- Creates get_gl_account_summary_filtered function using transaction_lines structure
...
-- Create the function using transaction_lines (line-item based structure)
...
AS $$
  WITH tx_expanded AS (
    -- Use transaction_lines table instead of direct transaction fields
    SELECT
      tl.transaction_id,
      t.entry_date,
      tl.org_id,
      ...
      (tl.debit_amount::numeric - tl.credit_amount::numeric) AS signed_amount
    ...
    WHERE TRUE
      ...
      AND (p_debit_account_id IS NULL OR (tl.debit_amount > 0 AND tl.account_id = p_debit_account_id))
      AND (p_credit_account_id IS NULL OR (tl.credit_amount > 0 AND tl.account_id = p_credit_account_id))
      AND (p_amount_min IS NULL OR (tl.debit_amount + tl.credit_amount) >= p_amount_min)
      AND (p_amount_max IS NULL OR (tl.debit_amount + tl.credit_amount) <= p_amount_max)
  ),
  opening AS (
    SELECT
      t.account_id,
...
GRANT EXECUTE ON FUNCTION public.get_gl_account_summary_filtered(
  date, date, uuid, uuid, boolean, integer, integer, uuid, uuid, uuid, uuid, uuid, uuid, numeric, numeric
...
-- Verification query
SELECT 
  'get_gl_account_summary_filtered function created successfully (using transaction_lines)' as status,
  now() as created_at;
