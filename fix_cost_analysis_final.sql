-- Final fix for cost analysis using correct table structure
-- Amount columns are in transaction_lines table, not transactions table
...
SELECT 
    ...
    t.entry_number,
    t.description,
    COALESCE(SUM(tl.debit_amount), SUM(tl.credit_amount), 0) AS transaction_amount,
    COALESCE(SUM(tli.total_amount), 0) AS calculated_line_items_total,
    COUNT(tli.id)::INTEGER AS calculated_line_items_count,
    (COALESCE(SUM(tl.debit_amount), SUM(tl.credit_amount), 0) - COALESCE(SUM(tli.total_amount), 0)) AS calculated_variance_amount,
    CASE 
        WHEN COALESCE(SUM(tl.debit_amount), SUM(tl.credit_amount), 0) = 0 THEN 0::NUMERIC
        ELSE ((COALESCE(SUM(tl.debit_amount), SUM(tl.credit_amount), 0) - COALESCE(SUM(tli.total_amount), 0)) / COALESCE(SUM(tl.debit_amount), SUM(tl.credit_amount), 0)) * 100
    END AS calculated_variance_pct,
    ABS(COALESCE(SUM(tl.debit_amount), SUM(tl.credit_amount), 0) - COALESCE(SUM(tli.total_amount), 0)) < 1.0 AS is_matched,
    ABS(COALESCE(SUM(tl.debit_amount), SUM(tl.credit_amount), 0) - COALESCE(SUM(tli.total_amount), 0)) >= 1.0 AS needs_attention,
...
GROUP BY t.id, t.entry_number, t.description;
