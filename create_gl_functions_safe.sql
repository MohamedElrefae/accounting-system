-- Safe creation of GL functions with expenses_category_id support
-- This script handles existing function conflicts properly
...
SELECT 
    ...
    p.oid,
    'DROP FUNCTION IF EXISTS public.' || p.proname || '(' || pg_get_function_identity_arguments(p.oid) || ');' as drop_statement
FROM pg_proc p
...
-- Step 2: Use CREATE OR REPLACE instead of CREATE to handle conflicts
-- This will overwrite any existing function with the same signature

-- Create/Replace get_general_ledger_report_filtered with expenses_category_id support
...
-- Create/Replace get_gl_account_summary_filtered with expenses_category_id support  
...
-- Step 4: Quick test (replace YOUR_ORG_ID with actual value)
-- SELECT COUNT(*) as test_count FROM get_general_ledger_report_filtered(p_limit => 5);
-- SELECT COUNT(*) as test_count FROM get_gl_account_summary_filtered(p_limit => 5);