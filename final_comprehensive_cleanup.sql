...
-- Remove ALL versions of get_gl_account_summary_filtered and create exactly one

begin;

-- Show what exists before cleanup
SELECT 'BEFORE CLEANUP: All existing function versions' as status;
SELECT 
  r.specific_name,
  r.routine_name,
...
-- AGGRESSIVE CLEANUP: Remove by every possible method
-- Method 1: Drop by specific function names
...
-- Method 2: Drop by all possible parameter combinations
DROP FUNCTION IF EXISTS public.get_gl_account_summary_filtered() CASCADE;
DROP FUNCTION IF EXISTS public.get_gl_account_summary_filtered(date) CASCADE;
DROP FUNCTION IF EXISTS public.get_gl_account_summary_filtered(date, date) CASCADE;
DROP FUNCTION IF EXISTS public.get_gl_account_summary_filtered(date, date, uuid) CASCADE;
DROP FUNCTION IF EXISTS public.get_gl_account_summary_filtered(date, date, uuid, uuid) CASCADE;
DROP FUNCTION IF EXISTS public.get_gl_account_summary_filtered(date, date, uuid, uuid, boolean) CASCADE;
DROP FUNCTION IF EXISTS public.get_gl_account_summary_filtered(date, date, uuid, uuid, boolean, integer) CASCADE;
...
-- Method 3: Drop with text parameters (old versions)
DROP FUNCTION IF EXISTS public.get_gl_account_summary_filtered(date, date, text, text, boolean, integer, integer) CASCADE;
DROP FUNCTION IF EXISTS public.get_gl_account_summary_filtered(date, date, text, text, text, boolean, integer, integer) CASCADE;
DROP FUNCTION IF EXISTS public.get_gl_account_summary_filtered(date, date, text, text, text, boolean, integer, integer, text) CASCADE;
DROP FUNCTION IF EXISTS public.get_gl_account_summary_filtered(date, date, text, text, text, boolean, integer, integer, text, text) CASCADE;
DROP FUNCTION IF EXISTS public.get_gl_account_summary_filtered(date, date, text, text, text, boolean, integer, integer, text, text, text) CASCADE;
DROP FUNCTION IF EXISTS public.get_gl_account_summary_filtered(date, date, text, text, text, boolean, integer, integer, text, text, text, text) CASCADE;

-- Method 4: Final catch-all
DROP FUNCTION IF EXISTS public.get_gl_account_summary_filtered CASCADE;

-- Create the ONE AND ONLY correct version
SELECT 'CREATING: The single correct function version' as status;
...
-- Final verification
SELECT 'AFTER CLEANUP: Functions that exist now' as status;
SELECT 
  r.specific_name,
  r.routine_name,
  string_agg(p.data_type, ', ' ORDER BY p.ordinal_position) as parameters
...
SELECT 'FINAL SUCCESS: Function overloading should be completely resolved' as final_status;
