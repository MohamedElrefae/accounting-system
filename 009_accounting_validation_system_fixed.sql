-- 009_accounting_validation_system_fixed.sql
...
-- FIXED VERSION: Handles account_category enum type
...
-- Fixed to handle account_category enum type by casting to text first
UPDATE public.accounts SET normal_balance = CASE 
  WHEN category::text IN ('asset', 'expense') THEN 'debit'
  WHEN category::text IN ('liability', 'equity', 'revenue') THEN 'credit'
...
BEGIN
  -- Get account details
  SELECT id, code, name, category::text as category, normal_balance, is_postable, status
  INTO debit_account
...
-- 8) Create RLS policies for validation logs
...
-- 11) Create additional RPC functions for API integration
