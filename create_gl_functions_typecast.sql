-- Final GL functions with explicit type casting to match return types
...
-- Create get_general_ledger_report_filtered with explicit type casting
...
BEGIN
    ...
    WITH expanded_entries AS (
        -- Debit entries
        SELECT 
            t.entry_number::text as t_entry_number,
            t.entry_date as t_entry_date,
            COALESCE(a.code, '')::text as t_account_code,
            COALESCE(a.name_ar, COALESCE(a.name, ''))::text as t_account_name_ar,
            COALESCE(a.name, COALESCE(a.name_ar, ''))::text as t_account_name_en,
            COALESCE(t.description, '')::text as t_description,
...
-- Create get_gl_account_summary_filtered with explicit type casting
...
SELECT 
    ...
    '✅ Function created with type casting' as status
FROM pg_proc p
...
-- Success message
SELECT '🎉 GL functions created successfully with expenses category filtering!' as success_message;