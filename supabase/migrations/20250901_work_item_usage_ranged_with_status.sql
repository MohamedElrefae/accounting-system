-- 20250901_work_item_usage_ranged_with_status.sql
-- Defines RPC get_work_item_usage_ranged with date range and posted/unposted filters
...
create or replace function public.get_work_item_usage_ranged(
  ...
  p_status text default 'all'  -- 'all' | 'posted' | 'unposted'
)
returns table (
  org_id uuid,
  project_id uuid,
...
begin
  ...
  with member_orgs as (
    ...
    where (
      ...
      p_org_id is null and v_uid is not null and v_has_org_memberships and exists (
        select 1 from public.org_memberships m where m.org_id = o.id and m.user_id = v_uid
      )
    )
  ), items as (
    ...
    where t.work_item_id is not null
      ...
      and (
        p_status = 'all'
        or (p_status = 'posted' and t.is_posted = true)
        or (p_status = 'unposted' and t.is_posted = false)
      )
