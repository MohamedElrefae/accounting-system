-- ============================================================================
-- Approval-Aware GL Summary Function (CORRECTED VERSION)
-- ============================================================================
-- This function integrates with the existing approval system that uses:
--   - transactions.approval_status field (NOT a separate table)
--   - approval_requests table for workflow management
--
-- Approval Status Values (from transactions.approval_status field):
--   - NULL or 'draft': Draft transactions
--   - 'submitted': Transaction submitted and awaiting approval
--   - 'approved': Transaction approved
--   - 'rejected': Transaction rejected
--
-- Usage:
--   - Pass NULL for p_approval_status to show ALL transactions
--   - Pass 'draft', 'submitted', 'approved', or 'rejected' to filter
-- ============================================================================
...
-- Create enhanced function with approval status support and dimension filtering
CREATE OR REPLACE FUNCTION public.get_gl_account_summary_filtered(
  ...
  p_approval_status text DEFAULT NULL,  -- 'draft', 'submitted', 'approved', 'rejected', or NULL for all
...
AS $$
  WITH tx_expanded AS (
    -- Join transaction_lines with transactions
    -- Filter by approval_status from transactions table (direct field)
    SELECT
      tl.transaction_id,
      t.entry_date,
      tl.org_id,
    ...
    WHERE TRUE
      ...
      AND (
        ...
        OR (p_approval_status = 'draft' AND (t.approval_status IS NULL OR t.approval_status = 'draft'))
        OR (p_approval_status != 'draft' AND t.approval_status = p_approval_status)
      )
...
-- Grant permissions (13 parameters total)
GRANT EXECUTE ON FUNCTION public.get_gl_account_summary_filtered(
  date, date, uuid, uuid, boolean, integer, integer, text, uuid, uuid, uuid, uuid, uuid
...
-- Test 3: Draft transactions only
...
-- Test 4: Submitted transactions (pending approval)
SELECT 'Test 4: Submitted transactions' as test_name;
SELECT 
  COUNT(*) as account_count,
  SUM(period_debits) as total_period_debit,
  SUM(period_credits) as total_period_credit,
...
FROM get_gl_account_summary_filtered(
  ...
  p_project_id := NULL,
  p_posted_only := false,
  p_approval_status := 'submitted'  -- Submitted only
);
