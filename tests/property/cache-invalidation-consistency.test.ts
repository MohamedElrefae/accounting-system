    ...
    await fc.assert(
      fc.asyncProperty(
        fc.record({
          ...
          otherUserId: fc.string({ minLength: 1, maxLength: 50 }),
        }),
        async (testData) => {
          // Feature: enterprise-auth-performance-optimization, Property 5: Cache Invalidation Consistency

          // Setup: Populate cache with role-related entries
          ...
          const otherUserAuthKey = cacheKeyStrategy.userAuth(testData.otherUserId);

          await cacheManager.set(userAuthKey, { userId: testData.userId, roles: [] }, CACHE_TTL.userAuth);
          await cacheManager.set(userRolesKey, { roles: ['admin'] }, CACHE_TTL.roles);
          await cacheManager.set(otherUserAuthKey, { userId: testData.otherUserId, roles: [] }, CACHE_TTL.userAuth);
