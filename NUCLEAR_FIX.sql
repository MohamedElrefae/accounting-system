-- ================================================
-- NUCLEAR FIX - Completely clear all functions and recreate
-- This removes ALL trial balance functions and creates clean ones
-- ================================================

-- Drop ALL possible function variants by name (ignore errors)
DO $$ 
DECLARE 
    func_record RECORD;
BEGIN
    -- Drop all functions that contain 'trial_balance' in the name
    FOR func_record IN 
    ...
    -- Drop all functions that contain 'account_balances' in the name
    FOR func_record IN 
        SELECT routine_name, routine_schema
        FROM information_schema.routines 
        WHERE routine_schema = 'public' 
        AND routine_name LIKE '%account_balances%'
    LOOP
        EXECUTE 'DROP FUNCTION IF EXISTS ' || func_record.routine_schema || '.' || func_record.routine_name || ' CASCADE';
    END LOOP;
END $$;

-- 1. Create the paginated trial balance function (transactions-based)
...
-- 2. Create the main trial balance function (transactions-based)
...
-- 3. Create as-of balance function (transactions-based)
...
-- Simple verification without function calls
...
SELECT '🚀 ALL FUNCTIONS RECREATED - TRY YOUR APP NOW! 🚀' as final_status;
