-- 009_accounting_validation_system_simple.sql
-- Comprehensive validation system to prevent backwards/incorrect journal entries
-- SIMPLIFIED VERSION: Works with existing schema without user_organizations table

BEGIN;

-- 1) Create function to validate transaction logic
...
BEGIN
  ...
  -- Check for backwards patterns based on normal balance vs transaction side
  -- If debiting a credit-normal account, it should have a good reason
  IF debit_account.normal_balance = 'credit' AND 
     NOT (p_description ILIKE '%دفع%' OR p_description ILIKE '%سداد%' OR p_description ILIKE '%تقليل%' OR p_description ILIKE '%تخفيض%') THEN
    RETURN QUERY SELECT TRUE, 'info'::TEXT,
      format('ملاحظة: خصم حساب ذو رصيد طبيعي دائن (%s) - تأكد من صحة العملية', debit_account.code),
      'عادة ما تكون هذه العملية لتقليل الرصيد أو السداد'::TEXT;
    RETURN;
  END IF;

  -- If crediting a debit-normal account, it should have a good reason  
  IF credit_account.normal_balance = 'debit' AND
     NOT (p_description ILIKE '%بيع%' OR p_description ILIKE '%تقليل%' OR p_description ILIKE '%استبعاد%' OR p_description ILIKE '%رد%') THEN
    RETURN QUERY SELECT TRUE, 'info'::TEXT,
      format('ملاحظة: إضافة دائنة لحساب ذو رصيد طبيعي مدين (%s) - تأكد من صحة العملية', credit_account.code),
      'عادة ما تكون هذه العملية لتقليل الرصيد أو الاسترداد'::TEXT;
    RETURN;
  END IF;
...
-- 2) Create trigger function for transaction validation
...
-- 3) Create validation trigger (only for unposted transactions)
...
-- 4) Create function to check account balance trends
...
-- 5) Create audit trail for validation overrides
...
-- 6) Create simplified RLS policies for validation logs
ALTER TABLE public.transaction_validation_logs ENABLE ROW LEVEL SECURITY;

-- Simple policy: users can only see validation logs for transactions they created
CREATE POLICY "Users can view their own validation logs" ON public.transaction_validation_logs
  FOR SELECT USING (
...
-- Users can insert validation logs for transactions in their organization
CREATE POLICY "Users can insert validation logs" ON public.transaction_validation_logs
  FOR INSERT WITH CHECK (
    validated_by = auth.uid()
  );

-- 7) Create function to log validation results
...
-- 8) Create view for transaction validation report
...
-- 9) Create additional RPC functions for API integration
