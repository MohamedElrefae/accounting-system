...
export type TransactionLineItem = {
  ...
  sub_tree_id?: string | null // Keep for backward compatibility
  expenses_category_id?: string | null // New column name
...
export async function upsertLineItems(transactionId: string, items: TransactionLineItem[]) {
  ...
  // Separate new items (no ID) from existing items (have ID)
  const newItems = items.filter(item => !item.id)
  const existingItems = items.filter(item => item.id)
  
  const results: TransactionLineItem[] = []
  
  // Insert new items (without ID, let database generate)
  if (newItems.length > 0) {
    const newPayload = newItems.map((it, idx) => {
      // Exclude generated/server-managed fields (total_amount will be calculated by trigger)
      const { total_amount, id, created_at, updated_at, ...itemWithoutManaged } = it
      return {
        ...itemWithoutManaged,
    ...
    if (newData) results.push(...newData as TransactionLineItem[])
  }
  
  // Update existing items (have ID)
  if (existingItems.length > 0) {
    const updatePayload = existingItems.map((it, idx) => {
      // Exclude generated/server-managed fields (total_amount will be calculated by trigger)
      const { total_amount, created_at, updated_at, ...itemWithoutManaged } = it
      return {
        ...itemWithoutManaged,
        transaction_id: transactionId,
    ...
    if (updateData) results.push(...updateData as TransactionLineItem[])
  }
