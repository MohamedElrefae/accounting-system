...
interface FontPreferencesContextType {
...
const FontPreferencesContext = createContext<FontPreferencesContextType | undefined>(undefined)
...
export const FontPreferencesProvider: React.FC<FontPreferencesProviderProps> = ({ children }) => {
      ...
      try {
        ...
        if (uid) {
          ...
          if (!hasNs && legacy) {
            ...
            try { localStorage.removeItem('font_prefs_v1') } catch {}
          }
        }
      } catch {}
  ...
  const contextValue: FontPreferencesContextType = {
    preferences,
    loading,
    error,
    refreshPreferences,
