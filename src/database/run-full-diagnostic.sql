-- Full diagnostic to see exact differences between reports
...
SELECT 
    ...
    tb.account_name_en,
    tb.account_type,
    tb.tb_debit as tb_debit,
    tb.tb_credit as tb_credit,
    ...
    CASE 
        WHEN tb.account_type IN ('assets', 'liabilities', 'equity') THEN
            CASE 
                ...
                ELSE '❌ BS DIFFER: ' || CAST(ABS((tb.tb_debit + tb.tb_credit) - bs.bs_amount) AS TEXT)
            END
        WHEN tb.account_type IN ('revenue', 'expenses') THEN
            CASE 
                ...
                ELSE '❌ PL DIFFER: ' || CAST(ABS((tb.tb_debit + tb.tb_credit) - pl.pl_amount) AS TEXT)
            END
        ELSE '➖ NOT BS/PL'
    END as match_status
