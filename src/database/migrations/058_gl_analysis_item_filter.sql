-- 058_gl_analysis_item_filter.sql
-- Extend GL summary and totals to accept Analysis Work Item filter

begin;

-- Update get_gl_account_summary to include p_analysis_work_item_id and filter by transactions.analysis_work_item_id
DROP FUNCTION IF EXISTS public.get_gl_account_summary(date, date, uuid, uuid, boolean, integer, integer, uuid);
DROP FUNCTION IF EXISTS public.get_gl_account_summary(date, date, uuid, uuid, boolean, integer, integer, uuid, uuid, uuid, uuid, uuid, uuid, numeric, numeric);
...
AS $$
  WITH tx_expanded AS (
    ...
    WHERE TRUE
      ...
      AND (p_analysis_work_item_id IS NULL OR tx.analysis_work_item_id = p_analysis_work_item_id)
    UNION ALL
    SELECT
      tx.id AS transaction_id,
      tx.entry_date,
...
GRANT EXECUTE ON FUNCTION public.get_gl_account_summary(
  date, date, uuid, uuid, boolean, integer, integer, uuid, uuid, uuid, uuid, uuid, uuid, numeric, numeric, uuid
) TO anon, authenticated, service_role;

-- Update get_gl_totals to include p_analysis_work_item_id and pass-through
DROP FUNCTION IF EXISTS public.get_gl_totals(date, date, uuid, uuid, boolean, uuid);
DROP FUNCTION IF EXISTS public.get_gl_totals(date, date, uuid, uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, numeric, numeric);
...
GRANT EXECUTE ON FUNCTION public.get_gl_totals(
  date, date, uuid, uuid, boolean, uuid, uuid, uuid, uuid, uuid, uuid, numeric, numeric, uuid
) TO anon, authenticated, service_role;
