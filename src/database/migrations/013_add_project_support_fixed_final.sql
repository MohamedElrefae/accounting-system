-- Fixed Final Project Support Migration
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' AND table_name = 'projects' AND column_name = 'budget_amount'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'unique_project_code_per_org'
        AND table_name = 'projects'
        AND table_schema = 'public'
    ) THEN
        ALTER TABLE projects ADD CONSTRAINT unique_project_code_per_org UNIQUE(org_id, code);
    END IF;
END$$;

-- 5. Add project_id to transactions table
...
-- 6. Add org_id to transactions table for consistency
...
-- 8. Enable RLS on projects table
...
-- 10. Insert sample projects - check which columns exist first
DO $$
DECLARE
    has_budget_amount boolean := false;
    has_start_date boolean := false;
    has_end_date boolean := false;
BEGIN
    -- Check if budget_amount column exists
    SELECT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' AND table_name = 'projects' AND column_name = 'budget_amount'
    ...
    IF has_budget_amount AND has_start_date AND has_end_date THEN
        -- Full insert with all columns
        INSERT INTO projects (code, name, description, status, budget_amount, start_date, end_date, created_by)
        SELECT 'GENERAL', 'عام', 'المشروع العام للمعاملات غير المخصصة لمشروع معين', 'active', NULL, NULL, NULL, auth.uid()
        WHERE NOT EXISTS (SELECT 1 FROM projects WHERE code = 'GENERAL')
        UNION ALL
        SELECT 'PROJ001', 'مشروع التطوير الأول', 'مشروع تطوير النظام المحاسبي', 'active', 100000.00, CURRENT_DATE, CURRENT_DATE + INTERVAL '1 year', auth.uid()
        WHERE NOT EXISTS (SELECT 1 FROM projects WHERE code = 'PROJ001')
        UNION ALL
        SELECT 'PROJ002', 'مشروع التسويق', 'حملة التسويق الرقمي', 'active', 50000.00, CURRENT_DATE, CURRENT_DATE + INTERVAL '6 months', auth.uid()
        WHERE NOT EXISTS (SELECT 1 FROM projects WHERE code = 'PROJ002');
    ELSIF has_budget_amount THEN
        -- Insert with budget_amount only
        ...
        WHERE NOT EXISTS (SELECT 1 FROM projects WHERE code = 'PROJ002');
    ELSE
        -- Basic insert without budget columns
        INSERT INTO projects (code, name, description, status, created_by)
        SELECT 'GENERAL', 'عام', 'المشروع العام للمعاملات غير المخصصة لمشروع معين', 'active', auth.uid()
        WHERE NOT EXISTS (SELECT 1 FROM projects WHERE code = 'GENERAL')
        UNION ALL
        SELECT 'PROJ001', 'مشروع التطوير الأول', 'مشروع تطوير النظام المحاسبي', 'active', auth.uid()
        WHERE NOT EXISTS (SELECT 1 FROM projects WHERE code = 'PROJ001')
        UNION ALL
        SELECT 'PROJ002', 'مشروع التسويق', 'حملة التسويق الرقمي', 'active', auth.uid()
        WHERE NOT EXISTS (SELECT 1 FROM projects WHERE code = 'PROJ002');
    END IF;
    
    RAISE NOTICE 'Sample projects inserted. Columns available: budget_amount=%, start_date=%, end_date=%', 
                 has_budget_amount, has_start_date, has_end_date;
END$$;

-- 11. Create view for project-based transaction reporting
...
SELECT 
  ...
  CASE 
    WHEN EXISTS (
      SELECT 1 FROM information_schema.columns 
      WHERE table_schema = 'public' AND table_name = 'projects' AND column_name = 'budget_amount'
    ) THEN p.budget_amount
    ELSE NULL
  END as project_budget
FROM transactions t
LEFT JOIN projects p ON t.project_id = p.id;

-- 12. Create function for project financial summary
  ...
  SELECT 
    ...
    CASE 
      WHEN EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' AND table_name = 'projects' AND column_name = 'budget_amount'
      ) AND p.budget_amount IS NOT NULL AND p.budget_amount > 0 
