-- Fixed Project Support Migration with Arabic Permissions
-- Execute this in the Supabase SQL Editor

-- 1. Add project permissions with Arabic translations (direct approach)
...
CREATE POLICY projects_update ON projects FOR UPDATE USING (created_by = auth.uid() OR public.has_permission(auth.uid(), 'projects.manage'));

DROP POLICY IF EXISTS projects_delete ON projects;
CREATE POLICY projects_delete ON projects FOR DELETE USING (created_by = auth.uid() OR public.has_permission(auth.uid(), 'projects.manage'));

-- 8. Insert sample projects
INSERT INTO projects (code, name, description, status, budget_amount, created_by)
VALUES 
  ('GENERAL', 'عام', 'المشروع العام للمعاملات غير المخصصة لمشروع معين', 'active', NULL, auth.uid()),
  ('PROJ001', 'مشروع التطوير الأول', 'مشروع تطوير النظام المحاسبي', 'active', 100000.00, auth.uid()),
  ('PROJ002', 'مشروع التسويق', 'حملة التسويق الرقمي', 'active', 50000.00, auth.uid())
