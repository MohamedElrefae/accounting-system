-- Test balance consistency with proper org_id filtering
-- Copy and paste this SQL block to verify the fix with org_id

WITH trial_balance_calculation AS (
    -- Replicate the reverted Trial Balance Original logic with org filtering
    SELECT 
        a.id as account_id,
        a.code,
        a.name,
    ...
    LEFT JOIN transactions t ON (
        ...
        AND t.org_id = 'bc16bacc-4fbe-4aeb-8ab1-fef2d895b441'  -- Use the correct org_id
    )
    WHERE a.org_id = 'bc16bacc-4fbe-4aeb-8ab1-fef2d895b441'  -- Filter accounts by org too
    GROUP BY a.id, a.code, a.name
),
gl_summary AS (
    -- Use the GL Summary function with the correct org_id
    SELECT 
        s.account_id,
        s.account_code,
    ...
    FROM public.get_gl_account_summary(
        ...
        'bc16bacc-4fbe-4aeb-8ab1-fef2d895b441'::uuid,     -- p_org_id (specific org)
        NULL,                                              -- p_project_id
        false,                                             -- p_posted_only
        NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL  -- other params
    ) s
),
comparison AS (
    SELECT 
...
-- Show remaining differences if any
SELECT 
    '=== REMAINING DIFFERENCES ===' as section_header,
    code,
    name,
    tb_debit,
    tb_credit,
