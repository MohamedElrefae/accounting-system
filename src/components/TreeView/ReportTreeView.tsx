...
const ReportTreeView: React.FC<ReportTreeViewProps> = ({
  ...
  const fmt = (n?: number | null) => Number(n || 0).toLocaleString('ar-EG');
  ...
  const renderNode = (node: ReportTreeNode, depth = 0): React.ReactNode => {
    ...
    const gridCols = gridColumnsForMode(mode, { withTxnCount: showTxnCount });
    ...
    return (
      <div key={node.id} className="tree-node-wrapper">
        <div className={`tree-node ${!node.is_active ? 'inactive' : ''} ${selectedId === node.id ? 'selected' : ''}`} style={{ gridTemplateColumns: gridCols }}>
          ...
          {mode === 'range' ? (
            <>
              <div className="tree-node-amount table-right">{fmt(node.opening_debit)}</div>
              <div className="tree-node-amount table-right">{fmt(node.opening_credit)}</div>
              <div className="tree-node-amount table-right">{fmt(node.period_debits)}</div>
              <div className="tree-node-amount table-right">{fmt(node.period_credits)}</div>
            </>
          ) : (
            <>
              <div className="tree-node-amount table-right">{fmt(node.closing_debit)}</div>
              <div className="tree-node-amount table-right">{fmt(node.closing_credit)}</div>
            </>
          )}
          ...
          <>
            ...
            {showSubtotals && (
              <div className="tree-node tree-subtotal" style={{ gridTemplateColumns: gridCols }}>
                ...
                {mode === 'range' ? (
                  <>
                    <div className="tree-node-amount table-right">{fmt(childSum(c => Number(c.opening_debit || 0)))}</div>
                    <div className="tree-node-amount table-right">{fmt(childSum(c => Number(c.opening_credit || 0)))}</div>
                    <div className="tree-node-amount table-right">{fmt(childSum(c => Number(c.period_debits || 0)))}</div>
                    <div className="tree-node-amount table-right">{fmt(childSum(c => Number(c.period_credits || 0)))}</div>
                  </>
                ) : (
                  <>
                    <div className="tree-node-amount table-right">{fmt(childSum(c => Number(c.closing_debit || 0)))}</div>
                    <div className="tree-node-amount table-right">{fmt(childSum(c => Number(c.closing_credit || 0)))}</div>
                  </>
                )}
                <div></div>
              </div>
  ...
  return (
    <div className="tree-view-container">
      <div className="tree-node tree-header" style={{ gridTemplateColumns: gridColumnsForMode(mode, { withTxnCount: showTxnCount }) }}>
      ...
      {showGrandTotal && (
        <div className="tree-node tree-grandtotal" style={{ gridTemplateColumns: gridColumnsForMode(mode, { withTxnCount: showTxnCount }) }}>
          <div></div>
          <div></div>
          <div></div>
          <div className="tree-node-code"></div>
          ...
          {mode === 'range' ? (
            <>
              <div className="tree-node-amount table-right">{fmt(grandTotals.opening_debit)}</div>
              <div className="tree-node-amount table-right">{fmt(grandTotals.opening_credit)}</div>
              <div className="tree-node-amount table-right">{fmt(grandTotals.period_debits)}</div>
              <div className="tree-node-amount table-right">{fmt(grandTotals.period_credits)}</div>
            </>
          ) : (
            <>
              <div className="tree-node-amount table-right">{fmt(grandTotals.closing_debit)}</div>
              <div className="tree-node-amount table-right">{fmt(grandTotals.closing_credit)}</div>
            </>
          )}
          <div></div>
        </div>
