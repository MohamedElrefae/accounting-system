...
export default function DocumentPermissionsDialog({ open, onClose, documentId }: Props) {
  const [loading, setLoading] = useState(false);
  const [rows, setRows] = useState<svc.DocumentPermissionInput[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setLoading(true);
        const perms = await svc.getDocumentPermissions(documentId);
        setExisting(perms);
  ...
  const updateRow = (idx: number, patch: Partial<svc.DocumentPermissionInput>) => {
    setRows(prev => prev.map((r, i) => (i === idx ? { ...r, ...patch } : r)));
  };

  const save = async () => {
    try {
      await svc.setDocumentPermissions(documentId, rows);
      const perms = await svc.getDocumentPermissions(documentId);
      setExisting(perms);
      setRows([]);
      showToast('تم حفظ صلاحيات المستند', { severity: 'success' });
    } catch (e: any) {
          ...
          {existing.map(row => (
            <Stack key={row.id} direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ py: 0.5, alignItems: 'center' }}>
              ...
              <Button size="small" color="error" onClick={async () => {
                try {
                  await svc.deleteDocumentPermission(row.id);
                  const perms = await svc.getDocumentPermissions(documentId);
                  setExisting(perms);
