...
The system implements automatic detection and recovery from IndexedDB corruption:

- **Automatic integrity checks** on database open to detect corruption early
- **Forensic data export** before recovery to preserve evidence for analysis
- **Server-based rebuild** with last 1000 transactions to restore user data
- **User notification** with data loss assessment and recovery status

### 6.2 Partial Sync Failure Recovery

Checkpoint-based synchronization ensures recovery from partial failures:

- **Checkpoint-based sync** (every 10 operations) to minimize rollback scope
- **Automatic rollback** to last checkpoint on failure to maintain consistency
- **Resume capability** from last successful checkpoint to avoid re-syncing
- **Operation-level undo** for rollback to reverse failed operations

### 6.3 Data Loss Prevention

Multiple layers of protection prevent accidental data loss:

- **Request persistent storage API** on first use to prevent browser eviction
- **Detect data loss** on startup by checking for missing IndexedDB
- **Warn user** before any unsynced data is lost with clear messaging
- **Maintain sync timestamp** in localStorage as canary for data loss detection

### 6.4 Bulk Rejection Handling

Intelligent handling of server-side validation failures:

- **Detect common rejection patterns** (closed period, invalid codes, etc.)
- **Offer bulk remediation options** for common scenarios (period reassignment)
- **Provide bulk edit interface** for manual fixes when needed
- **Never silently discard** rejected operations - always require user decision
