-- Quick fix for p_sub_tree_id column error
-- Update the function to remove any references to p_sub_tree_id

begin;

-- Drop and recreate the function without p_sub_tree_id references
...
CREATE OR REPLACE FUNCTION public.get_gl_account_summary_filtered(
  ...
  p_sub_tree_id uuid DEFAULT NULL  -- Keep parameter but don't use it
)
RETURNS TABLE (
  account_id uuid,
  account_code text,
...
AS $$
  WITH tx_expanded AS (
    ...
    WHERE TRUE
      ...
      -- Note: p_sub_tree_id parameter is ignored since the column doesn't exist in transaction_lines
  ),
  opening AS (
    SELECT
      t.account_id,
...
-- Verification
SELECT 'SUCCESS: Fixed p_sub_tree_id column error' as status;
SELECT 
  routine_name,
  routine_type
