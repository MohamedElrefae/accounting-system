-- GL functions based on the actual database schema
-- Your database uses a single 'transactions' table with double-entry structure

-- First verify the transactions table exists
SELECT 'Transactions table found' as status, COUNT(*) as row_count FROM transactions LIMIT 1;
...
-- Create get_general_ledger_report_filtered with correct schema
...
-- Create get_gl_account_summary_filtered with correct schema
...
SELECT 
    ...
    '✅ Function created with correct schema' as status
FROM pg_proc p
