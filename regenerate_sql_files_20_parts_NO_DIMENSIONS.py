#!/usr/bin/env python3
"""
Regenerate 20 SMALL SQL import files from the corrected CSV WITHOUT ACCOUNTING DIMENSIONS.
This will produce SQL files with ~700 lines each to avoid Supabase SQL Editor timeout.

DIMENSIONS EXCLUDED (will be added in a follow-up UPDATE script):
- classification_id
- project_id
- analysis_work_item_id
- sub_tree_id

This allows immediate import. Dimensions can be added later using code→UUID mapping.
"""
...
def regenerate_sql_files():
    """Regenerate 20 SMALL SQL files from corrected CSV WITHOUT dimensions."""
    ...
    print(f"\n=== GENERATING {NUM_FILES} SMALL SQL FILES WITHOUT DIMENSIONS ===")
    print(f"Lines per file: ~{lines_per_file} (smaller files for Supabase SQL Editor)")
    print(f"⚠️  NOTE: Dimensions will be added in a follow-up UPDATE script")
    ...
    for file_num in range(1, NUM_FILES + 1):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"-- WITHOUT ACCOUNTING DIMENSIONS (will be added later)\n")
    ...
    print(f"\n💡 NEXT STEPS:")
    print(f"   1. Import transactions first: import_transactions.sql")
    print(f"   2. Import all 20 transaction_lines files")
    print(f"   3. Run dimension update script (will be created separately)")
