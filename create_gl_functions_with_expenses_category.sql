-- Create/Update General Ledger functions to support expenses_category_id filtering
-- Run these SQL statements in your database

-- First, let's check what functions exist (run this query first to see current state)
SELECT 
...
-- Drop existing functions if they exist (without expenses_category_id parameter)
-- Comment out any that don't exist to avoid errors
...
-- Create updated get_general_ledger_report_filtered with expenses_category_id support
...
-- Create updated get_gl_account_summary_filtered with expenses_category_id support  
...
-- Test the functions (replace with actual values)
-- SELECT * FROM get_general_ledger_report_filtered(
--     p_org_id => 'your-org-id-here',
--     p_limit => 5
-- );

-- SELECT * FROM get_gl_account_summary_filtered(
--     p_org_id => 'your-org-id-here', 
--     p_limit => 5
-- );