#!/usr/bin/env python3
"""
Regenerate 5 SQL import files from the corrected CSV WITH ACCOUNTING DIMENSIONS.
...
def get_dimension_uuid(df, dimension_code, dimension_type):
    """Get UUID for a dimension code from database mapping."""
    # For now, return the code as-is (will need to map to UUIDs later)
    # Empty string for NULL values
    if pd.isna(dimension_code) or dimension_code == '':
        return 'NULL'
    return f"'{dimension_code}'"

def regenerate_sql_files():
    """Regenerate 5 SQL files from corrected CSV with dimensions."""
    ...
    print(f"\n=== DIMENSION COLUMNS ===")
    print(f"classification_id: {df['classification_id'].notna().sum()} non-null values")
    print(f"project_id: {df['project_id'].notna().sum()} non-null values")
    print(f"analysis_work_item_id: {df['analysis_work_item_id'].notna().sum()} non-null values")
    print(f"sub_tree_id: {df['sub_tree_id'].notna().sum()} non-null values")
    ...
    print(f"\n=== GENERATING {NUM_FILES} SQL FILES WITH DIMENSIONS ===")
    ...
    for file_num in range(1, NUM_FILES + 1):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"-- WITH ACCOUNTING DIMENSIONS\n")
            ...
            for idx, row in chunk.iterrows():
                ...
                # Handle dimension values (use actual values from CSV, or NULL if empty)
                classification = f"'{row['classification_id']}'" if pd.notna(row['classification_id']) and row['classification_id'] != '' else 'NULL'
                project = f"'{row['project_id']}'" if pd.notna(row['project_id']) and row['project_id'] != '' else 'NULL'
                analysis = f"'{row['analysis_work_item_id']}'" if pd.notna(row['analysis_work_item_id']) and row['analysis_work_item_id'] != '' else 'NULL'
                sub_tree = f"'{row['sub_tree_id']}'" if pd.notna(row['sub_tree_id']) and row['sub_tree_id'] != '' else 'NULL'
                
                values_row = f"        ({row_num}, '{txn_ref}', '{row['account_id']}', {classification}, {project}, {analysis}, {sub_tree}, {row['debit_amount']}, {row['credit_amount']}, '{desc}', '{ORG_ID}')"
    ...
    for i in range(1, NUM_FILES + 1):
        print(f"   - import_transaction_lines_part_{i:02d}.sql")
