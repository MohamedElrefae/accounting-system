#!/usr/bin/env python3
"""
Regenerate 20 SMALL SQL import files from the corrected CSV WITH ACCOUNTING DIMENSIONS.
This will produce SQL files with ~700 lines each to avoid Supabase SQL Editor timeout.
...
# Number of files to generate (20 files = ~700 lines each)
NUM_FILES = 20

def regenerate_sql_files():
    """Regenerate 20 SMALL SQL files from corrected CSV with dimensions."""
    ...
    print(f"\n=== GENERATING {NUM_FILES} SMALL SQL FILES WITH DIMENSIONS ===")
    print(f"Lines per file: ~{lines_per_file} (smaller files for Supabase SQL Editor)")
    ...
    for file_num in range(1, NUM_FILES + 1):
        ...
        print(f"✓ Part {file_num:02d}: {len(chunk):,} lines | Debit: {chunk_debit:,.2f} | Credit: {chunk_credit:,.2f}")
    ...
    print(f"   Total: {NUM_FILES} files (~{lines_per_file} lines each)")
    print(f"\n💡 TIP: These smaller files should work better in Supabase SQL Editor")
